...
class CaptchaManager:
    @staticmethod
    async def send_captcha(bot_name, user, captcha_type, message_url, data, expires_in = None, token_hash = None):
        """
        ...
        expires_in (int, optional): Time in seconds until the captcha expires. Defaults to None (no expiration).
        """
        ...
        try:
            CaptchaStore.add(bot_name, payload)
            count = CaptchaStore.count()

            socketio.emit('captcha_new', payload, namespace = '/')
            socketio.emit('captcha_count', {'count': count}, namespace = '/')
        ...
        except Exception: logger.exception('Failed to dispatch captcha to web')
