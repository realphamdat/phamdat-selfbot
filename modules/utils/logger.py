...
class WebSocketHandler(logging.Handler):
    def __init__(self, max_buffer = 300):
        super().__init__()
        self.buffer = deque(maxlen=max_buffer)
        self.socketio = None

    def set_socketio(self, sio): self.socketio = sio
    ...
    def emit(self, record):
        try:
            ...
            if self.socketio: self.socketio.emit('log', entry, namespace='/')
        except Exception: self.handleError(record)

    def get_buffer(self): return list(self.buffer)
