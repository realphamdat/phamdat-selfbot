...
class Topgg:
	...
	async def vote_topgg(self):
		...
		async with webdriver.Chrome(options = options) as driver:
			...
			await asyncio.sleep(20)
			button = await driver.find_element(by = By.XPATH, value = f'//a[@href="/bot/{self.client.data.bot.id}/vote"]')
			await button.click()
			self.client.logger.info(f"Loaded {self.client.data.bot.display_name} vote page on top.gg")
			await asyncio.sleep(20)
			button = await driver.find_element(by=By.XPATH, value=".//button[contains(text(),'Vote')]")
			await button.click()
			self.client.logger.info(f"Voted {self.client.data.bot.display_name} on top.gg")
			await asyncio.sleep(20)