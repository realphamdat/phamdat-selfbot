"""
Huntbot: claim, submit, solve password via template matching.
"""
...
from modules.owo.constants import OWO_HUNTBOT_DIR

class Huntbot:
    @staticmethod
    async def solve_password(captcha_url):
        """Solve huntbot password image via template matching."""
        templates = []
        import os
        for img_path in sorted(glob.glob(os.path.join(OWO_HUNTBOT_DIR, '**', '*.png'), recursive=True)):
    ...
    async def claim_submit(client):
        ...
        try:
            ...
            # Password not included, get reset time
