...
class Gamble:
    ...
    async def check_slot(client, message):
        ...
        if str(client.nickname) not in message.content: return
    ...
    async def lottery(client):
        ...
        if not cfg['mode']: return
        if client.cooldown_lottery - time.time() > 0: return
    ...
    async def play_slot(client):
        ...
        if client.bet_slot >= max_bet: client.bet_slot = int(client.config['gamble']['slot']['bet'])
    ...
    async def play_coinflip(client):
        ...
        if client.bet_coinflip >= max_bet: client.bet_coinflip = int(client.config['gamble']['coinflip']['bet'])
    ...
    async def play_blackjack(client):
        ...
        if client.bet_blackjack >= max_bet: client.bet_blackjack = int(client.config['gamble']['blackjack']['bet'])
    ...
    async def gamble_cycle(client):
        ...
        except Exception: client.logger.exception('Gamble error')