"""
Spam module (replaces grind): hunt, battle, owo/uwu, random messages.
"""
...
class Spam:
    ...
    async def send_random_message(client):
        """Send a random message wrapped in backticks."""
        if not client.selfbot_running or not client.current_channel:
            return
        msg = random.choice(client.random_messages)
        await client.current_channel.send(f'`{msg}`')

    @staticmethod
    async def spam_cycle(client):
        """Run one spam cycle: send enabled commands with delays."""
